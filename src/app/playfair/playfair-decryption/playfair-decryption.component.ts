...
@Component({
  selector: 'app-playfair-decryption',
  templateUrl: './playfair-decryption.component.html',
  styleUrls: ['./playfair-decryption.component.css']
})
export class PlayfairDecryptionComponent implements OnInit {
  ...
  constructor(
    public dialogRef: MatDialogRef<PlayfairDecryptionComponent>,
    @Inject(MAT_DIALOG_DATA) public data: DialogData) {
      this.Key = data.Key;
      this.CipherText = data.Text;
      this.PlainText = "";
      let msg = this.CipherText.toUpperCase();
      let M = msg[0]
      ...
      if(msg.length%2!=0)
        msg+="X";
      this.CipherTextMatrix = [];
      for(let i=0;i<msg.length;i+=2)
        this.CipherTextMatrix.push(msg[i]+msg[i+1]);
  ...
  PlayDecryption(){
    ...
    this.takeFourNumbers = interval(parseInt(this.Speed)*6).pipe(
      startWith(0),
      take(this.CipherTextMatrix.length),
      finalize(()=>{
        setTimeout( () => {   
          this.Play = false;
        }, parseInt(this.Speed)*this.CipherTextMatrix.length);
      })
    );
    ...
    this.Running  = this.takeFourNumbers.subscribe(x => {
      ...
      let x1, x2, y1, y2, a, b;
      a = this.CipherTextMatrix[x][0];
      b = this.CipherTextMatrix[x][1];
      if(a=='J' || a=='I')
        a = 'I/J'
      if(b=='J' || b=='I')
        b = 'I/J'
      ...
      this.Running1  =  this.takeFourNumber.subscribe(i => {
        ...
        if(i==0){
          ...
          if(x1 == x2){
            rows[x1].getElementsByTagName("td")[(y2-1+5)%5].className = "Activate";
            this.PlainText+=this.KeyMatrix[x1][(y1-1+5)%5];
          }
          else if(y1 == y2){
            rows[(x1-1+5)%5].getElementsByTagName("td")[y1].className = "Activate";
            this.PlainText+=this.KeyMatrix[(x1-1+5)%5][y1];
          }
          else{
            rows[x1].getElementsByTagName("td")[y2].className = "Activate";
            this.PlainText+=this.KeyMatrix[x1][y2];
          }
        }
        else if(i==3){
          rows[x1].getElementsByTagName("td")[y1].className = "DeActivate";
          if(x1 == x2){
            rows[x1].getElementsByTagName("td")[(y2-1+5)%5].className = "DeActivate";
          }
          else if(y1 == y2){
            rows[(x1-1+5)%5].getElementsByTagName("td")[y1].className = "DeActivate";
          }
          else{
          ...
          if(x1 == x2){
            rows[x2].getElementsByTagName("td")[(y2-1+5)%5].className = "Activate";
            this.PlainText+=this.KeyMatrix[x1][(y1-1+5)%5]+this.KeyMatrix[x2][(y2+1)%5];
          }
          else if(y1 == y2){
            rows[(x2-1+5)%5].getElementsByTagName("td")[y2].className = "Activate";
            this.PlainText+=this.KeyMatrix[(x2-1+5)%5][y2];
          }
          else{
            rows[x2].getElementsByTagName("td")[y1].className = "Activate";
            this.PlainText+=this.KeyMatrix[x2][y1];
          }
